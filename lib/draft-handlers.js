 ...
 * Includes the collaborative roots themselves plus all transitive composition children.
 * Used so that draft operations on child entities (e.g. OrderItems) also participate
 * in collaborative InProcessByUser pre-setting and WS event emission.
 * @param {object} srv
  ...
  function walkCompositions(entityDef) {
    ...
    for (const comp of Object.values(comps)) {
      ...
      for (const [shortName, e] of Object.entries(srv.entities || {})) {
        if (e.name === target.name) { subtree.add(shortName); break }
      }
...
function registerHandlers(srv, collaborativeEntities) {
  ...
  srv.handle = async function collabDraftHandle(req) {
    // For collaborative draft entities (including composition children like OrderItems),
    ...
    // Root entity changes are handled by the after('UPDATE') hook; this covers composition
    // children (e.g. creating/patching/deleting OrderItems inside a collaborative Order draft).
  ...
  srv.on('READ', 'DraftAdministrativeUser', async function onReadDraftAdminUser(req) {
    ...
    // Extract UserID exclusion filters from the OData $filter (e.g. "UserID ne 'alice'").
    // FE sends this to exclude the current user from the discard dialog participant list.
 ...
 * Queries each collaborative entity's drafts table for a row with the given DraftUUID.
 * @param {string} draftUUID
 * @param {object} srv
 * @param {Set<string>} collaborativeEntities
 * @returns {Promise<{entitySetName: string, entityID: string}|null>}
 */
