 ...
 */
export function isCollaborativeDraftEnabled(entity: any): boolean {
 ...
 */
export function findCollaborativeDraftEntities(csn: any): string[] {
  const result: string[] = []
  for (const [name, def] of Object.entries<any>(csn.definitions || {})) {
 ...
 */
export function augmentCompiledModel(model: any): void {
  ...
  const _linkedProto: object | null = (() => {
    for (const d of Object.values<any>(model.definitions)) {
      if (d.kind === 'entity' && typeof d.set === 'function') return Object.getPrototypeOf(d)
    }
    return null
  })()

  function _makeLinkedEntity(props: Record<string, unknown>): any {
  ...
  for (const [name, def] of Object.entries<any>(model.definitions)) {
    ...
    const rawActionName: string = def['@Common.DraftRoot.ShareAction']
    ...
    if (!def.actions[actionShortName]) {
      ...
      params[Symbol.iterator] = function* (this: any) { yield* Object.values(this) }
