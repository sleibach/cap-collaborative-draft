...
cds.on('bootstrap', (app) => {
        ...
        res.send = function (body) {
            ...
            // "invalid segment: DraftMessages" when FE tries to navigate to it.
            {
                ...
                while ((esMatch = esRegex.exec(body)) !== null) {
                    ...
                    if (!etMatch || etMatch[0].includes('NavigationProperty Name="DraftMessages"'))
                        continue;
                    body = body.replace(etRegex, (_full, etContent, closing) => `${etContent}        <NavigationProperty Name="DraftMessages" Type="Collection(${ns}.DraftMessages)" ContainsTarget="true"/>\n    ${closing}`);
                    changed = true;
                }
            }
            if (changed)
    ...
    app.use(async (req, res, next) => {
        ...
        try {
            ...
            if (!draftUUID)
                return next();
            const stateRow = await db.run(SELECT.one.from('DRAFT.CollaborativeDraftState').columns('DraftAccessType').where({ DraftUUID: draftUUID }));
            if (stateRow?.DraftAccessType !== '3')
                return next();
        ...
        try {
            ...
            if (wsService) {
                ...
                wsService.on('MESSAGE', async (msg) => {
                    ...
                    let clientContent = d.clientContent || '';
